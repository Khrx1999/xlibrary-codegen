...
- **Self-healing locators** — when alternative selectors are available, every step carries an inline `# xlib:step=N;alts=[...]` comment with up to 3 ranked candidates and a letter grade (A+ to D). The live viewer shows colored grade chips per step. (v0.2.1: bare `# xlib:step=N` markers are no longer emitted — direct-mode output stays clean.)
...
Re-record one or more steps in an existing generated file. If the file has `xlib:step=N` markers, the step index is used directly; otherwise `patch` falls back to fuzzy content-match on the keyword line (case-insensitive substring) and prints a disambiguation table when multiple steps match.
...
> 1.0** — pin an exact version (`xlibrary@0.2.1`) if you rely on it.
